...
def update_value_util(st, ss, se, i, diff, si):
    ...
    if (se != ss):
        mid = get_mid(ss, se)
        update_value_util(st, ss, mid, i,
                          diff, 2 * si + 1)
        update_value_util(st, mid + 1, se, i,
                          diff, 2 * si + 2)
...
def construct_st(array, length_of_array):
    # Allocate memory for the segment tree

    # Height of segment tree
    x = int(ceil(log2(length_of_array)))
    ...
    construct_st_util(array, 0, length_of_array - 1, st, 0)
